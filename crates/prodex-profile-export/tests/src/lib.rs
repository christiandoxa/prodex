...
fn profile_export_envelope_rejects_oversized_bundle_before_reading() {
    ...
    write_profile_export_bundle(&path, b"{}")
        .expect("private bundle should be created before extending it");
    std::fs::OpenOptions::new()
        .write(true)
        .open(&path)
        .expect("private bundle should reopen")
...
fn profile_export_bundle_private_io_round_trips() {
    ...
    let content = serialize_profile_export_payload(&serde_json::json!({ "profile": "main" }), None)
        .expect("payload should serialize");

    write_profile_export_bundle(&path, &content).expect("private bundle should be written");
    let (envelope, encrypted) = read_profile_export_envelope::<serde_json::Value>(&path)
        .expect("private bundle should be read");

    assert!(!encrypted);
    assert!(matches!(
        envelope,
        ProfileExportEnvelope::Plain { payload, .. }
            if payload == serde_json::json!({ "profile": "main" })
    ));
    let _ = std::fs::remove_dir_all(root);
}
...
fn profile_export_bundle_uses_no_follow_private_atomic_io() {
    ...
    let err = read_profile_export_envelope::<serde_json::Value>(&path)
        .expect_err("symlink bundle should be rejected");
    assert!(format!("{err:#}").contains("failed to read"));

    write_profile_export_bundle(&path, b"inside").expect("symlink should be safely replaced");
    ...
    write_profile_export_bundle(&path, b"rotated").expect("bundle should atomically rotate");
...
fn profile_export_bundle_rejects_public_files_without_echoing_contents() {
    ...
    let err = read_profile_export_envelope::<serde_json::Value>(&path)
        .expect_err("public bundle should be rejected before parsing");
...
fn profile_export_bundle_rejects_reparse_point_reads() {
    ...
    let err = read_profile_export_envelope::<serde_json::Value>(&path)
        .expect_err("reparse point bundle should be rejected");
    assert!(format!("{err:#}").contains("failed to read"));
    assert_eq!(std::fs::read_to_string(target).unwrap(), "outside");
    let _ = std::fs::remove_dir_all(root);
}

fn profile_export_private_temp_dir(name: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
    ...
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).expect("private temp dir should be created");
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt as _;

        std::fs::set_permissions(&root, std::fs::Permissions::from_mode(0o700))
            .expect("private temp dir mode should be set");
    }
    root
}
