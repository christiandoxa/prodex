...
pub(crate) fn gemini_runtime_responses_value_from_generate_value(
    ...
    created_at: u64,
    default_model: &str,
    blocked_tool_call_message: impl FnMut(&str, &Value) -> Option<String>,
) -> Value {
...
pub(crate) fn gemini_runtime_responses_value_from_generate_value_with_fallback_ids(
    value: &Value,
    created_at: u64,
    default_model: &str,
    mut blocked_tool_call_message: impl FnMut(&str, &Value) -> Option<String>,
    fallback_response_id: impl FnOnce() -> String,
    mut fallback_call_id: impl FnMut(usize) -> String,
) -> Value {
    ...
    gemini_build_response_value(
        ...
        |part, function_call, index| {
            let call_id =
                gemini_function_call_id_with_fallback(function_call, || fallback_call_id(index));
