...
fn gemini_provider_core_shapes_response_stream_events() {
    ...
    assert_eq!(
        gemini_provider_core_output_item_done_event(7, None, &json!({"type": "web_search_call"})),
        json!({
            "type": "response.output_item.done",
            "sequence_number": 7,
...
fn gemini_provider_core_shapes_stream_function_call_arguments_delta() {
    ...
    assert!(!gemini_provider_core_stream_should_emit_function_call_arguments_delta("tool_search"));
    assert!(!gemini_provider_core_stream_should_emit_function_call_arguments_delta("apply_patch"));
    assert_eq!(
