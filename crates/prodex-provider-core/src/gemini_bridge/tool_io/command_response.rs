//! Gemini command-tool response shaping.

pub fn gemini_provider_core_structured_command_tool_response(
    tool_name: &str,
    output: &str,
) -> Option<serde_json::Value> {
    if !gemini_provider_core_is_command_tool_name(tool_name) || output.trim().is_empty() {
        return None;
    }
    if let Some(session_id) =
        gemini_provider_core_u64_after_marker(output, "Process running with session ID")
    {
    ...
    if let Some(exit_code) =
        gemini_provider_core_i64_after_marker(output, "Process exited with code")
    {
        ...
        if exit_code != 0 {
            gemini_provider_core_add_command_failure_guidance(&mut response, output);
        }
...
fn gemini_provider_core_is_command_tool_name(tool_name: &str) -> bool {
...
fn gemini_provider_core_u64_after_marker(text: &str, marker: &str) -> Option<u64> {
...
fn gemini_provider_core_i64_after_marker(text: &str, marker: &str) -> Option<i64> {
    let tail = text.split_once(marker)?.1.trim_start();
...
fn gemini_provider_core_add_command_failure_guidance(
    ...
    if let Some(path) = gemini_provider_core_extract_missing_path(output)
        .or_else(|| gemini_provider_core_extract_clone_target(output))
    {
...
fn gemini_provider_core_extract_missing_path(output: &str) -> Option<String> {
    gemini_provider_core_quoted_value_after_marker(output, "cannot access").or_else(|| {
        gemini_provider_core_quoted_value_after_marker(output, "No such file or directory")
    })
}

fn gemini_provider_core_extract_clone_target(output: &str) -> Option<String> {
    gemini_provider_core_quoted_value_after_marker(output, "Cloning into")
}

fn gemini_provider_core_quoted_value_after_marker(output: &str, marker: &str) -> Option<String> {
