...
Prodex registers `prodex-sqz` when `sqz-mcp` is on `PATH`, and `prodex-token-savior` when `token-savior` is on `PATH`. Missing binaries are skipped silently so Super still launches cleanly.
