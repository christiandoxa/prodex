...
mod rust_oracle {
    use super::super::estimation::smart_context_estimate_tokens_from_body_bytes_rust;
    use super::super::*;
    use crate::RuntimeTokenUsage;
    ...
    pub(in crate::smart_context) const SMART_CONTEXT_ADAPTIVE_ESTIMATE_RECENT_USAGE_LIMIT: usize =
        4;
    pub(in crate::smart_context) const SMART_CONTEXT_ADAPTIVE_ESTIMATE_MAX_INFLATION: u64 = 2;

    pub(in crate::smart_context) fn smart_context_observed_calibrated_request_estimate_rust(
        body_bytes: usize,
        ...
        if calibrated
            > baseline_estimate.saturating_mul(SMART_CONTEXT_ADAPTIVE_ESTIMATE_MAX_INFLATION)
        {
            baseline_estimate
        } else {
            calibrated
            ...
            SmartContextTokenCalibrationFallbackTier::RouteTransportGlobal => sample
                .map(|sample| {
                    (smart_context_token_calibration_field_matches(&target.route, &sample.route)
                        ...
                        || smart_context_token_calibration_sample_is_global_compatible(
                            target, sample,
                        )
                })
                .unwrap_or(true),
    ...
    pub(in crate::smart_context) fn smart_context_token_calibration_model_matches(
        ...
        let Some(target) = smart_context_token_calibration_normalized_model(target.as_deref())
        else {
            return false;
        };
        let Some(sample) = smart_context_token_calibration_normalized_model(sample.as_deref())
        else {
            return false;
        };
        target == sample
    ...
    pub(in crate::smart_context) fn smart_context_token_calibration_sample_is_global_compatible(
        ...
        smart_context_token_calibration_optional_field_compatible(&target.route, &sample.route)
            && smart_context_token_calibration_optional_model_compatible(
                &target.model,
                &sample.model,
            )
    ...
    pub(in crate::smart_context) fn smart_context_token_calibration_model_family(
        model: &str,
    ) -> &str {
        let model = model
            .strip_suffix("-latest")
...
#[cfg(any(not(feature = "mojo"), test))]
pub(in crate::smart_context) use rust_oracle::*;
