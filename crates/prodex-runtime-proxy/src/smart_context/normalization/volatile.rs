...
pub fn smart_context_normalize_volatile_command_output(text: &str) -> Cow<'_, str> {
    ...
    rust_oracle::smart_context_normalize_volatile_command_output_rust(text)
}
...
pub fn smart_context_normalize_volatile_static_context(text: &str) -> Cow<'_, str> {
    ...
    #[cfg(not(feature = "mojo"))]
    rust_oracle::smart_context_normalize_volatile_static_context_rust(text)
}
#[cfg(not(feature = "mojo"))]
mod rust_oracle {
    use super::super::*;
    ...
    pub(super) fn smart_context_normalize_volatile_command_output_rust(text: &str) -> Cow<'_, str> {
        ...
        while index < text.len() {
            ...
            if let Some((len, replacement)) =
                smart_context_command_output_replacement(rest, previous)
            {
    ...
    fn smart_context_command_output_replacement(
        ...
        if let Some((len, replacement)) =
            smart_context_labeled_random_id_replacement(text, previous)
        {
    ...
    pub(super) fn smart_context_normalize_volatile_static_context_rust(text: &str) -> Cow<'_, str> {
        let mut normalized = String::with_capacity(text.len());
        let mut changed = false;
        let mut index = 0usize;
    ...
    pub(in crate::smart_context) fn smart_context_percent_progress_len(
        text: &str,
    ) -> Option<usize> {
    ...
    pub(in crate::smart_context) fn smart_context_uuid_len(
        ...
        if !smart_context_uuid_token_exact(candidate)
            || !smart_context_after_token_boundary(text, 36)
        {
            return None;
        }
        Some(36)
    ...
    pub(in crate::smart_context) fn smart_context_random_id_value_looks_volatile_for_key(
        ...
        (hex_like && value.len() >= 16)
            || (alpha && digit && (value.len() >= 16 || entropy_marks > 0))
    }
    ...
    pub(in crate::smart_context) fn smart_context_skip_ascii_spaces(
        text: &str,
        start: usize,
    ) -> usize {
        let bytes = text.as_bytes();
    ...
    pub(in crate::smart_context) fn smart_context_ascii_case_prefix(
        text: &str,
        prefix: &str,
    ) -> bool {
        text.get(..prefix.len())
    ...
    pub(in crate::smart_context) fn smart_context_after_token_boundary(
        ...
        text[index..].chars().next().is_none_or(|ch| {
            !(ch.is_ascii_alphanumeric() || matches!(ch, '_' | '-' | '.' | ':' | '/'))
        })
    }
}
