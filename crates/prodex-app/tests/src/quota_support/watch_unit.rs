    ...
    fn test_quota_report_with_provider(
        ...
        QuotaReport {
            ...
            active: false,
            auth: AuthSummary {
                label: match _provider {
                    ProfileProvider::Copilot { .. } => "copilot".to_string(),
                    ProfileProvider::Gemini { .. } => "gemini".to_string(),
                    ProfileProvider::Anthropic { .. } => "anthropic".to_string(),
                    ProfileProvider::Kiro { .. } => "kiro".to_string(),
                    _ => "chatgpt".to_string(),
                },
    ...
    fn quota_watch_tui_fallback_message_redacts_secret_like_error() {
        ...
        assert!(message.starts_with(
            "prodex quota TUI unavailable, falling back to plain watch: quota TUI startup failed"
        ));
    ...
    fn all_quota_watch_current_sort_places_active_profile_first() {
        ...
        assert!(output.contains("* active-late"));
    }
    ...
    #[test]
    fn openai_locked_detail_watch_can_update_log_header_cache() {
        assert!(quota_watch_runtime_usage_cache_enabled(
            true,
            &QuotaAuthFilter::All,
