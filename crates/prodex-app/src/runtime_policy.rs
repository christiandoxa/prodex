...
pub(super) use prodex_runtime_policy::{
    RuntimeLogFormat, RuntimePolicySummary, ensure_runtime_policy_valid, runtime_policy_runtime,
...
pub fn deliver_pending_config_publication_events_to_gateway_runtime(
    ...
    deliver_pending_config_publication_events_with_activation(transport_root, replica, root, |_| {
        Ok(())
    })
}

pub fn deliver_pending_config_publication_events_with_activation(
    transport_root: &Path,
    replica: &str,
    root: &Path,
...
#[cfg(test)]
#[path = "runtime_policy/tests/publication.rs"]
mod tests;
