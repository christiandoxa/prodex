use super::{
    ...
    runtime_proxy_structured_log_message, runtime_quota_last_chance_profile_for_route,
    runtime_response_trace_provider_labels, runtime_smart_context_model_name_from_body,
...
fn forward_runtime_response_processed_websocket_request(
    session_id: u64,
    request_id: u64,
    request_text: &str,
...
fn runtime_response_processed_response_id(request_text: &str) -> Option<String> {
    let value = serde_json::from_str::<serde_json::Value>(request_text).ok()?;
    if value.get("type").and_then(serde_json::Value::as_str) != Some("response.processed") {
        return None;
    }
    Some(
        value
...
struct RuntimeWebsocketTextMessageFlow<'a> {
    ...
    quota_last_chance_profile: Option<String>,
    reset_selection_budget: bool,
    saw_inflight_saturation: bool,
    local_capacity_wait_timed_out: bool,
