...
pub(crate) use wait::{
    ...
    runtime_profile_inflight_wait_outcome_since,
...
fn runtime_proxy_request_has_owned_lane_affinity(
    ...
    if lane == RuntimeRouteKind::Compact {
        if let Some(turn_state) = runtime_request_turn_state(request) {
            let compact_lineage_key = runtime_compact_turn_state_lineage_key(&turn_state);
            if runtime
                .turn_state_bindings
                .get(turn_state.as_str())
                .or_else(|| runtime.turn_state_bindings.get(&compact_lineage_key))
                .is_some_and(binding_profile_exists)
            {
                return true;
            }
