...
use super::info::{runtime_logs_json_value, runtime_policy_json_value, secret_backend_json_value};
use crate::{
...
pub(crate) fn handle_info(args: InfoArgs) -> Result<()> {
    ...
    if args.json {
        ...
        let value = serde_json::json!({
            ...
            "providers": format_info_provider_summary(&state.profiles),
            "provider_routes": format_info_provider_capabilities_summary(&state.profiles),
            "runtime_policy": runtime_policy_json_value(policy_summary.as_ref()),
            ...
            "fields": field_values,
            "quota": {
                "quota_compatible_profiles": quota.quota_compatible_profiles,
                "live_profiles": quota.live_profiles,
                "snapshot_profiles": quota.snapshot_profiles,
                "unavailable_profiles": quota.unavailable_profiles,
                "five_hour_profiles_with_data": quota.five_hour_profiles_with_data,
                "weekly_profiles_with_data": quota.weekly_profiles_with_data,
                "five_hour_pool_remaining": quota.five_hour_pool_remaining,
                "weekly_pool_remaining": quota.weekly_pool_remaining,
                "earliest_five_hour_reset_at": quota.earliest_five_hour_reset_at,
                "earliest_weekly_reset_at": quota.earliest_weekly_reset_at,
            },
            "runtime_load": {
                "log_count": runtime_load.log_count,
                "active_inflight_units": runtime_load.active_inflight_units,
                "recent_selection_events": runtime_load.recent_selection_events,
            },
        });
