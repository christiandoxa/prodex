...
#[test]
fn existing_cloudflare_endpoint_keeps_hostname_exact_and_binds_loopback_port() {
    assert_eq!(
        validate_existing_cloudflare_hostname("Prodex.Example.com").unwrap(),
        "prodex.example.com"
    );
    for invalid in [
        "https://prodex.example.com",
        "user:password@prodex.example.com",
        "prodex.example.com/path",
        "localhost",
        "127.0.0.1",
    ] {
        assert!(
            validate_existing_cloudflare_hostname(invalid).is_err(),
            "{invalid}"
        );
    }
    ...
    .unwrap_err()
    .to_string();
    assert!(error.contains("LocalOriginPortInUse"), "{error}");
}
