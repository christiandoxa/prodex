...
pub(crate) const GEMINI_OAUTH_DISABLED_GUIDANCE: &str = "Google Gemini OAuth profiles are unsupported and disabled. For Codex-fronted Gemini, migrate to a Gemini API key (`--api-key`, `GEMINI_API_KEY`, or `GOOGLE_API_KEY`). Native Gemini CLI / Vertex AI compatibility was retired; use the Gemini provider bridge with API-key authentication.";
