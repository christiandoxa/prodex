...
pub(super) const GEMINI_COMPAT_FILE_LIMIT: usize = 512 * 1024;
const GENERATED_MARKER: &str = "prodex-gemini-cli-compat";
pub(super) const GENERATED_PROMPT_MARKER: &str =
...
#[derive(Debug, Clone)]
pub(super) struct GeminiExtension {
...
pub(super) fn extension_skill_dirs(extension: &GeminiExtension) -> Vec<PathBuf> {
...
pub(super) fn write_toml_table(path: &Path, table: toml::Table, context: &str) -> Result<()> {
...
pub(super) fn toml_string(value: &toml::Value, keys: &[&str]) -> Option<String> {
    keys.iter()
        .find_map(|key| value.get(*key).and_then(toml::Value::as_str))
        .map(str::to_string)
}

pub(super) fn toml_string_array(value: &toml::Value, keys: &[&str]) -> Vec<String> {
    keys.iter()
