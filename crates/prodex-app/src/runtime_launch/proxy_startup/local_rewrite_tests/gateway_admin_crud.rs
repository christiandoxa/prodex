...
fn gateway_admin_can_create_rotate_disable_and_delete_virtual_keys() {
    ...
    assert_eq!(
        ...
        serde_json::json!(["native", "minimal", "evaluated"])
    );
    assert_eq!(providers["default_harness_mode"], "native");
    assert_eq!(providers["resolved_harness_mode"], "native");
    assert_eq!(providers["harness_modes"][1]["id"], "minimal");
    assert_eq!(providers["harness_modes"][1]["request_shaping"], true);
    assert_eq!(providers["harness_modes"][1]["response_shaping"], false);
    assert_eq!(providers["harness_modes"][1]["stream_shaping"], false);
