use super::governance_bundle::RuntimeGovernanceRequestSnapshot;
use super::{
...
pub(super) fn runtime_gateway_load_compatible_governance_bundle(
    ...
    for _ in 0..GOVERNANCE_BUNDLE_LOAD_ATTEMPTS {
        ...
        let policy_status = &before[0];
        let mut candidates = Vec::with_capacity(2);
        if let Some(active) = policy_status.active_revision_id.as_deref() {
            candidates.push(active);
        }
        if let Some(last_known_good) = policy_status.last_known_good_revision_id.as_deref()
            && !candidates.contains(&last_known_good)
        {
            candidates.push(last_known_good);
        }

        for policy_revision in candidates {
            let stored_policy = match runtime_gateway_load_governance_snapshot(
                authority,
                sqlite_repository,
                tenant_id,
                prodex_storage::GovernanceArtifactKind::Policy,
                ...
                Err(error) => return Err(governance_load_error(&error)),
            };
            let policy =
            ...
            if !governance_status_has_revision(
                &before[1],
                pinned_classification.revision().as_str(),
            ...
            let stored_classification = match runtime_gateway_load_governance_snapshot(
                authority,
                sqlite_repository,
                tenant_id,
                prodex_storage::GovernanceArtifactKind::ClassificationRules,
                |input| {
                    input.revision_id == pinned_classification.revision().as_str()
                        ...
                        .is_ok_and(|snapshot| {
                            snapshot.classification_rules().checksum()
            ...
            let stored_provider = match runtime_gateway_load_governance_snapshot(
                authority,
                sqlite_repository,
                tenant_id,
                prodex_storage::GovernanceArtifactKind::ProviderRegistry,
            ...
            let stored_routing = match runtime_gateway_load_governance_snapshot(
                authority,
                sqlite_repository,
                tenant_id,
                prodex_storage::GovernanceArtifactKind::RoutingScores,
            ...
            if before
                == runtime_gateway_governance_statuses(authority, sqlite_repository, tenant_id)?
            {
                return Ok(RuntimeGovernanceRequestSnapshot {
...
pub(super) fn spawn_runtime_gateway_governance_refresh_worker(
    ...
    Some(thread::spawn(move || {
        ...
        while !shutdown.load(Ordering::SeqCst) {
            ...
            let tenant_ids = match discovered.and_then(|discovered| {
            ...
            let mut classification_unavailable = 0usize;
            let mut provider_refreshed = 0usize;
            let mut provider_unavailable = 0usize;
            let mut routing_refreshed = 0usize;
            let mut routing_unavailable = 0usize;
            for tenant_id in &tenant_ids {
                let refresh = authority.commit_for_tenant(*tenant_id, || {
                        let mut next = (*governance_snapshots.load_full()).clone();
                        if deployment_mode.is_enforcing() {
                            return match runtime_gateway_load_compatible_governance_bundle(
                                &authority,
                                ...
                                *tenant_id,
                                ...
                                Err(
                                    ...
                                    if let Some(updated) =
                                        next.without_tenant_snapshot(*tenant_id)
                                    {
                        ...
                        let (policy_refreshed, policy_unavailable) =
                            runtime_gateway_refresh_policy_snapshot(
                                &authority,
                                sqlite_repository.as_ref(),
                                *tenant_id,
                                &governance_policy,
                        ...
                        match runtime_gateway_load_governance_snapshot(
                            ...
                            *tenant_id,
                            ...
                            Ok(stored) => {
                                if let Ok(snapshot) = super::super::local_rewrite_classification_rules::compile_runtime_classification_rules_artifact(
                                    *tenant_id,
                                    ...
                                    && let Ok(updated) = next.classification.with_tenant_snapshot(*tenant_id, snapshot)
                                {
                            ...
                            Err(error) => {
                                classification_unavailable = runtime_gateway_invalidate_unavailable_snapshot(
                                    &error,
                                    &mut next.classification,
                                    |snapshots| snapshots.without_tenant_snapshot(*tenant_id),
                                );
                            }
                        }
                        let mut provider_refreshed = 0usize;
                            ...
                            *tenant_id,
                            ...
                            Ok(stored) => {
                                if let Ok(snapshot) = super::super::local_rewrite_provider_registry::compile_runtime_gateway_provider_registry_artifact_for_deployment(
                                    ...
                                    && let Ok(updated) = next.provider_registry.with_tenant_snapshot(*tenant_id, snapshot)
                                {
                            ...
                            Err(error) => {
                                provider_unavailable = runtime_gateway_invalidate_unavailable_snapshot(
                                    &error,
                                    &mut next.provider_registry,
                                    |snapshots| snapshots.without_tenant_snapshot(*tenant_id),
                                );
                            }
                        }
                            ...
                            *tenant_id,
                            ...
                            Ok(stored) => {
                                if let Ok(snapshot) = super::super::local_rewrite_provider_registry::compile_runtime_gateway_routing_scores_artifact(
                                    ...
                                    && let Ok(updated) = next.routing_scores.with_tenant_snapshot(*tenant_id, snapshot)
                                {
                            ...
                            Err(error) => {
                                routing_unavailable = runtime_gateway_invalidate_unavailable_snapshot(
                                    &error,
                                    &mut next.routing_scores,
                                    |snapshots| snapshots.without_tenant_snapshot(*tenant_id),
                                );
                            }
                        }
                ...
                {
                    policy_refreshed += refreshed_policy;
                    policy_unavailable += unavailable_policy;
                    classification_refreshed += refreshed_classification;
                    classification_unavailable += unavailable_classification;
                    provider_refreshed += refreshed_provider;
                    provider_unavailable += unavailable_provider;
                    routing_refreshed += refreshed_routing;
                    routing_unavailable += unavailable_routing;
                }
            }
            if policy_refreshed
                + policy_unavailable
                ...
                > 0
            {
                runtime_proxy_log_to_path(
                    &log_path,
                    &format!(
                        "governance_snapshot_refresh status=success policy={policy_refreshed} policy_unavailable={policy_unavailable} classification_rules={classification_refreshed} classification_rules_unavailable={classification_unavailable} provider_registry={provider_refreshed} provider_registry_unavailable={provider_unavailable} routing_scores={routing_refreshed} routing_scores_unavailable={routing_unavailable} configured={}",
                        tenant_ids.len()
                    ),
                );
            } else {
            ...
            for _ in 0..50 {
                if shutdown.load(Ordering::SeqCst)
                    || governance_refresh_requested.swap(false, Ordering::AcqRel)
                {
                    break;
                }
                thread::sleep(Duration::from_millis(100));
