...
pub(super) fn runtime_deepseek_repair_tool_call_adjacency(messages: &mut Vec<serde_json::Value>) {
    ...
    for mut message in std::mem::take(messages) {
        ...
        repaired.push(super::runtime_deepseek_normalize_assistant_tool_call_content(message));
