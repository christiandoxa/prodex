...
pub(super) fn runtime_gateway_respond_route_explain(
    ...
    preauthorized_admin: Option<RuntimeGatewayAdminAuth>,
) {
    let captured = match capture_runtime_proxy_request_with_limit(
        &mut request,
    ...
    let response = runtime_gateway_admin_response(0, &captured, shared, preauthorized_admin)
        .unwrap_or_else(|| {
