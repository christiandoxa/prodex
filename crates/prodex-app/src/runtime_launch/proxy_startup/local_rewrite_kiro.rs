...
mod stream_driver;
use self::messages::runtime_kiro_messages_streaming_upstream_result;
use self::process::{
...
pub(super) fn send_runtime_kiro_upstream_request(
    ...
    if stream_mode == Streaming {
        if messages_route {
            return runtime_kiro_messages_streaming_upstream_result(context);
        }
...
fn runtime_kiro_buffered_upstream_result(
    ...
    (|| {
        ...
        let body = if chat_completions_route {
            ...
            .context("failed to serialize Kiro Anthropic Messages JSON")?
        } else {
