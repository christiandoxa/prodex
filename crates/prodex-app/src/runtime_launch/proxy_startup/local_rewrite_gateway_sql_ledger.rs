...
pub(super) fn runtime_gateway_sqlite_ledger_load(
    ...
    let sql = if runtime_gateway_sqlite_ledger_has_scope_columns(&conn)? {
        r#"
        ...
        r#"
        SELECT phase, request_id, NULL AS typed_request_id, call_id, key_name,
               NULL AS tenant_id, NULL AS team_id, NULL AS project_id, NULL AS user_id,
               NULL AS budget_id, model, minute_epoch, input_tokens,
               estimated_cost_microusd, created_at_epoch, response_status, response_bytes,
               output_tokens, final_cost_microusd, final_cost_usd, reconciled_at_epoch
        FROM prodex_gateway_billing_ledger
        ORDER BY id DESC
        LIMIT ?1
        "#
...
pub(super) fn runtime_gateway_postgres_ledger_load(
    ...
    let sql = if runtime_gateway_postgres_ledger_has_scope_columns(&mut client)? {
        r#"
        SELECT phase, request_id, typed_request_id, call_id, key_name, tenant_id,
               team_id, project_id, user_id, budget_id, model, minute_epoch,
               input_tokens, estimated_cost_microusd, created_at_epoch,
...
fn runtime_gateway_sqlite_ledger_has_scope_columns(conn: &rusqlite::Connection) -> Result<bool> {
...
mod tests {
    ...
    #[test]
    fn sqlite_legacy_ledger_schema_still_accepts_usage_deltas() {
    ...
    #[test]
    fn sqlite_ledger_reconcile_matches_call_id_not_colliding_request_id() {
