...
#[test]
fn versioned_sidecar_cleanup_rejects_outside_root() {
    let root = temp_root("sidecar-cleanup-containment");
    let outside = root.with_extension("outside");
    let path = outside.join("runtime.json");
    let backup_path = root.join("runtime.json.last-good");
    fs::create_dir_all(&outside).expect("outside directory should be created");
    fs::write(&path, "primary").expect("primary should be writable");
    fs::write(&backup_path, "last-good").expect("backup should be writable");

    let error = remove_versioned_json_file_with_backup_under(&root, &path, &backup_path)
        .expect_err("cleanup should reject paths outside its root");
