...
const GEMINI_CODE_ASSIST_OPERATION_POLL_INTERVAL: Duration = Duration::from_secs(5);
const GEMINI_CODE_ASSIST_OPERATION_TIMEOUT: Duration = Duration::from_secs(5 * 60);
...
fn setup_gemini_code_assist_project_with_endpoint(
    ...
    operation = wait_for_gemini_code_assist_operation(
        client,
        secret,
        code_assist_endpoint,
        operation,
        GEMINI_CODE_ASSIST_OPERATION_POLL_INTERVAL,
        GEMINI_CODE_ASSIST_OPERATION_TIMEOUT,
    )?;
...
fn wait_for_gemini_code_assist_operation(
    client: &Client,
    secret: &GeminiOAuthSecret,
    code_assist_endpoint: &str,
    mut operation: GeminiCodeAssistOperationResponse,
...
mod tests {
    ...
    #[test]
    fn code_assist_onboarding_operation_has_an_overall_deadline() {
        let server = TinyServer::http("127.0.0.1:0").expect("poll test server should bind");
        let listen_addr = server.server_addr().to_ip().unwrap();
        let endpoint = format!("http://{listen_addr}/v1internal");
        let secret = test_gemini_secret(None);
        let handle = thread::spawn(move || {
        ...
        let err = wait_for_gemini_code_assist_operation(
            &client,
            &secret,
            &endpoint,
            GeminiCodeAssistOperationResponse {
                name: Some("operations/pending".to_string()),
                done: Some(false),
                response: None,
            },
        ...
        handle.join().expect("poll test server should finish");

        assert!(err.to_string().contains("onboarding timed out"));
    }
