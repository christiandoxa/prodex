...
use prodex_mojo_core::context::{
    CONTEXT_OUTPUT_DIAGNOSTIC_TARGET, CONTEXT_OUTPUT_ESLINT,
    CONTEXT_OUTPUT_FAILURE, CONTEXT_OUTPUT_JUNIT_FAILURE, CONTEXT_OUTPUT_NOISY_KEY,
    CONTEXT_OUTPUT_RUST_EXIT, CONTEXT_OUTPUT_RUST_NOISE, CONTEXT_OUTPUT_WARNING,
};

#[cfg(not(feature = "mojo"))]
mod rust_classification_prefix {
    use super::rust_classification::rust_test_noise_label;
    use super::*;
    ...
    pub(crate) fn rust_block_line_limit(options: &CommandOutputCompactOptions) -> usize {
        options.max_lines.max(24).saturating_div(5).clamp(8, 32)
    }

    pub(crate) fn diagnostic_block_line_limit(options: &CommandOutputCompactOptions) -> usize {
        options.max_lines.max(24).saturating_div(5).clamp(8, 32)
    }

    pub(crate) fn rust_noise_label(line: &str) -> Option<&'static str> {
...
#[cfg(not(feature = "mojo"))]
pub(crate) use rust_classification_prefix::{
    diagnostic_block_line_limit, looks_like_diagnostic_output, looks_like_noisy_success_output,
    looks_like_rust_diagnostic_output, rust_block_line_limit, rust_noise_label,
};

#[cfg(not(feature = "mojo"))]
pub(crate) use rust_classification::{diagnostic_noise_label, noisy_success_label};
...
pub(crate) fn rust_noise_label(line: &str) -> Option<&'static str> {
    ...
    (flags & CONTEXT_OUTPUT_RUST_NOISE != 0
        && flags & (CONTEXT_OUTPUT_FAILURE | CONTEXT_OUTPUT_WARNING) == 0
        && !is_error_signal_line(line)
        && !is_test_failure_signal_line(line))
    .then(|| mojo_output_label(result.noisy_label))
    .flatten()
}

#[cfg(feature = "mojo")]
pub(crate) fn diagnostic_noise_label(line: &str) -> Option<&'static str> {
    mojo_output_label(command_output_line_classification(line).diagnostic_label)
}

#[cfg(feature = "mojo")]
pub(crate) fn noisy_success_label(line: &str) -> Option<&'static str> {
    mojo_output_label(command_output_line_classification(line).noisy_label)
}
...
pub(crate) fn looks_like_rust_diagnostic_output(lines: &[&str]) -> bool {
    ...
    let mut backtraces = 0usize;
    let mut exits = 0usize;
    let mut clippy = 0usize;
    for line in lines {
        if rust_diagnostic_severity(line).is_some()
            || rust_failed_test_name(line).is_some()
            || rust_failure_separator_name(line).is_some()
...
#[cfg(not(feature = "mojo"))]
mod rust_classification {
    use super::*;

    pub(crate) fn rust_test_noise_label(trimmed: &str) -> Option<&'static str> {
    ...
    fn noisy_success_registry_package_label(lower: &str) -> Option<&'static str> {
        ...
        } else if lower.starts_with("lockfile is up to date")
