...
pub(super) fn compact_structured_json_output(
    ...
    let candidate = prodex_mojo_core::context::compact_structured_json(
        input,
        count_text_lines(input),
        options.max_line_chars,
    )
    .expect("Mojo structured-JSON compactor returned invalid output")?;
    finalize_mojo_structured_json_compaction(input, candidate, options)
}
...
pub(super) fn compact_structured_json_output(
    ...
    rust_compat::compact_structured_json_output_rust(input, options)
}

#[cfg(feature = "mojo")]
fn finalize_mojo_structured_json_compaction(
    original: &str,
    candidate: String,
    options: &CommandOutputCompactOptions,
) -> Option<String> {
    let candidate = ensure_mojo_structured_json_critical_signals(original, &candidate, options);
...
#[cfg(feature = "mojo")]
fn ensure_mojo_structured_json_critical_signals(
    original: &str,
    candidate: &str,
    options: &CommandOutputCompactOptions,
) -> String {
...
mod rust_compat {
    ...
    pub(super) fn compact_structured_json_output_rust(
        input: &str,
        options: &CommandOutputCompactOptions,
    ) -> Option<String> {
    ...
    fn finalize_structured_json_compaction(
        ...
        if candidate.len() < original.len()
            && critical_signal_self_check(original, &candidate).passed()
        {
            Some(candidate)
        } else {
            None
