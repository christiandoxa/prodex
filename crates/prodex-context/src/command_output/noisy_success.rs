...
pub(crate) use rust_output::{
    ...
    is_noisy_success_key_line, is_success_output_failure_signal_line,
    is_success_output_warning_signal_line, is_typescript_diagnostic_line,
};

#[cfg(feature = "mojo")]
#[path = "noisy_success/mojo.rs"]
...
pub(crate) mod rust_output {
    ...
    pub(crate) fn is_dot_reporter_success_line(trimmed: &str) -> bool {
        trimmed.len() >= 4 && trimmed.chars().all(|ch| ch == '.')
    }

    pub(crate) fn is_bun_test_success_line(trimmed: &str, lower: &str) -> bool {
        lower.starts_with("bun test v")
            || lower.starts_with("(pass) ")
            || lower.starts_with("ran ")
                && lower.contains(" tests across ")
                && lower.contains(" file")
    ...
    pub(crate) fn is_swift_test_success_line(lower: &str) -> bool {
    ...
    pub(crate) fn is_zig_test_success_line(lower: &str) -> bool {
        lower == "test"
    ...
    pub(crate) fn is_gradle_test_success_line(trimmed: &str, lower: &str) -> bool {
    ...
    pub(crate) fn is_maven_test_success_line(_trimmed: &str, lower: &str) -> bool {
    ...
    pub(crate) fn is_package_install_success_line(lower: &str) -> bool {
    ...
    pub(crate) fn is_docker_buildx_success_line(lower: &str) -> bool {
        lower.starts_with('#')
    ...
    pub(crate) fn is_bazel_test_success_line(lower: &str) -> bool {
    ...
    pub(crate) fn zero_count_summary_line(lower: &str, word: &str) -> bool {
        lower.match_indices(word).any(|(index, matched)| {
            count_after_word(lower, index + matched.len())
                .or_else(|| count_before_word(lower, index))
                == Some(0)
        })
    }

    pub(crate) fn is_count_word_line(lower: &str, expected_word: &str) -> bool {
    ...
    pub(crate) fn is_typescript_success_line(trimmed: &str, lower: &str) -> bool {
    ...
    pub(crate) fn looks_like_typescript_project_line(trimmed: &str) -> bool {
        let trimmed = trimmed.trim_matches(|ch| matches!(ch, '\'' | '"' | '`' | ',' | ';'));
        trimmed.ends_with("tsconfig.json") || trimmed.ends_with("tsconfig.tsbuildinfo")
    }

    pub(crate) fn is_vite_success_line(trimmed: &str, lower: &str) -> bool {
    ...
    pub(crate) fn is_next_success_line(trimmed: &str, lower: &str) -> bool {
    ...
    pub(crate) fn is_next_route_table_line(trimmed: &str, lower: &str) -> bool {
        matches!(
            trimmed.chars().next(),
            Some('┌' | '├' | '└' | '│' | '○' | '●' | 'ƒ' | '+')
    ...
    pub(crate) fn is_coverage_noise_line(trimmed: &str, lower: &str) -> bool {
    ...
    pub(crate) fn looks_like_pytest_coverage_table_row(trimmed: &str, lower: &str) -> bool {
    ...
    pub(crate) fn is_junit_xml_success_line(trimmed: &str, lower: &str) -> bool {
    ...
    pub(crate) fn is_junit_xml_failure_line(line: &str) -> bool {
        ...
        trimmed.starts_with("<failure")
            ...
            || ((trimmed.starts_with("<testsuite") || trimmed.starts_with("<testsuites"))
                && (xmlish_nonzero_attr(&lower, "failures")
                    || xmlish_nonzero_attr(&lower, "errors")))
    }

    pub(crate) fn xmlish_nonzero_attr(lower: &str, name: &str) -> bool {
        for needle in [
            format!("{name}=\""),
            format!("{name}='"),
            format!("{name}="),
    ...
    pub(crate) fn is_playwright_success_line(trimmed: &str, lower: &str) -> bool {
    ...
    pub(crate) fn is_cypress_success_line(trimmed: &str, lower: &str) -> bool {
    ...
    pub(crate) fn is_biome_success_summary_line(lower: &str) -> bool {
    ...
    pub(crate) fn is_oxlint_success_summary_line(lower: &str) -> bool {
    ...
    pub(crate) fn is_pytest_progress_line(line: &str) -> bool {
    ...
    pub(crate) fn is_pytest_success_summary_line(lower: &str) -> bool {
        lower.contains(" passed")
            && lower.contains(" in ")
    ...
    pub(crate) fn is_diagnostic_success_summary_line(line: &str) -> bool {
    ...
    pub(crate) fn is_common_success_summary_line(lower: &str) -> bool {
        lower.starts_with("build successful")
            || lower.contains(" build success")
            || lower.starts_with("[info] build success")
            || lower.contains("actionable tasks:")
    ...
    pub(crate) fn is_nonzero_fail_count_line(lower: &str) -> bool {
    ...
    pub(crate) fn is_diagnostic_detection_start(line: &str) -> bool {
        is_typescript_diagnostic_line(line)
            || is_eslint_diagnostic_line(line)
            || is_junit_xml_failure_line(line)
            || is_exception_signal_line(line)
