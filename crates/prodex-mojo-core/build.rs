...
fn emit_source_rerun_directives(sources: &[&str], manifest_dir: &Path) {
    ...
    if sources.iter().any(|source| source.contains("rich_")) {
        for source in [
            ...
            "../../mojo/prodex_core/context_command_output_search.mojo",
        ] {
            println!(
                "cargo:rerun-if-changed={}",
                manifest_dir.join(source).display()
...
fn selected_sources() -> Vec<&'static str> {
        ...
        || env::var_os("CARGO_FEATURE_MOJO_CORE").is_some()
    {
        ...
        sources.push("../../mojo/prodex_core/quota_pool.mojo");
    }
        ...
        || env::var_os("CARGO_FEATURE_MOJO_CORE").is_some()
    {
        ...
        sources.push("../../mojo/prodex_core/context_blob_noise.mojo");
        ...
        sources.push("../../mojo/prodex_core/continuation_status.mojo");
