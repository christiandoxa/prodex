...
mod rust_compat {
    ...
    use super::super::runtime_doctor_top_facet;

    pub(crate) fn runtime_doctor_selection_pressure(summary: &RuntimeDoctorSummary) -> String {
    ...
    pub(crate) fn runtime_doctor_transport_pressure(summary: &RuntimeDoctorSummary) -> String {
    ...
    pub(crate) fn runtime_doctor_persistence_pressure(summary: &RuntimeDoctorSummary) -> String {
    ...
    pub(crate) fn runtime_doctor_startup_audit_pressure(summary: &RuntimeDoctorSummary) -> String {
    ...
    pub(crate) fn runtime_doctor_quota_freshness_pressure(
        summary: &RuntimeDoctorSummary,
    ) -> String {
        if summary.stale_persisted_usage_snapshots > 0
            || runtime_doctor_marker_count(summary, RuntimeDoctorMarker::ProfileProbeRefreshError)
                > 0
        ...
        } else if runtime_doctor_has_any_markers(
            summary,
            RUNTIME_DOCTOR_ACTIVE_QUOTA_REFRESH_MARKERS,
        ) {
            "active".to_string()
        } else {
            "low".to_string()
