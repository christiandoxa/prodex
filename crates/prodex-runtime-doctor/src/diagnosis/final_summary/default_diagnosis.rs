...
mod rust_compat {
    ...
    use super::super::super::broker::runtime_doctor_broker_issue_diagnosis;
    use super::super::super::marker_accessors::*;
    use super::super::super::next_steps::*;
    use super::super::compact::runtime_doctor_compact_exit_counts;
    ...
    pub(crate) fn runtime_doctor_default_diagnosis(summary: &RuntimeDoctorSummary) -> String {
        ...
        if summary.line_count == 0 {
            return "Latest runtime log is empty.".to_string();
        }
        if let Some(diagnosis) = runtime_doctor_broker_issue_diagnosis(summary) {
            return diagnosis;
        }

        let diagnosers: [fn(&RuntimeDoctorSummary) -> Option<String>; 8] = [
            runtime_doctor_proxy_pressure_diagnosis,
            runtime_doctor_websocket_connect_diagnosis,
            runtime_doctor_profile_pressure_diagnosis,
            runtime_doctor_continuation_diagnosis,
            runtime_doctor_compatibility_diagnosis,
            runtime_doctor_provider_diagnosis,
            runtime_doctor_transport_diagnosis,
            runtime_doctor_trailing_diagnosis,
        ];
        diagnosers
            .into_iter()
            .find_map(|diagnose| diagnose(summary))
            .unwrap_or_else(|| {
    ...
    fn runtime_doctor_proxy_pressure_diagnosis(summary: &RuntimeDoctorSummary) -> Option<String> {
        if runtime_doctor_marker_count(summary, "runtime_proxy_overload_backoff") > 0 {
            return Some("Recent local proxy overload backoff was triggered.".to_string());
        }
        if runtime_doctor_marker_count(summary, "runtime_proxy_lane_limit_reached") > 0 {
            let lane = runtime_doctor_marker_last_field(
                summary,
        ...
        if runtime_doctor_marker_count(summary, "runtime_proxy_active_limit_reached") > 0 {
            return Some(format!(
                "Recent global active-request admission limit was triggered. Next step: {}",
                runtime_doctor_active_pressure_next_step(summary)
            ));
        }
        if runtime_doctor_marker_count(summary, "runtime_proxy_queue_overloaded") > 0 {
            return Some("Recent proxy saturation detected before commit.".to_string());
        }
        if runtime_doctor_marker_count(summary, "profile_circuit_open") > 0 {
            return Some("Recent route-level circuit breaker opened; fresh selection is temporarily steering away from a degraded profile.".to_string());
        }
        if runtime_doctor_marker_count(summary, "profile_circuit_half_open_probe") > 0 {
            return Some("Recent route-level circuit breaker entered half-open probing; fresh selection is cautiously testing a degraded profile before fully restoring it.".to_string());
        }
        if runtime_doctor_marker_count(summary, "websocket_precommit_frame_timeout") > 0 {
            return Some("Recent websocket reuse/connect path failed to produce a first upstream frame before the pre-commit deadline.".to_string());
        }
        if runtime_doctor_marker_count(summary, "websocket_precommit_hold_timeout") > 0 {
            return Some(
                "Recent websocket pre-commit hold timed out before an upstream terminal frame arrived."
                    .to_string(),
            );
        }
        if runtime_doctor_marker_count(summary, "websocket_dns_resolve_timeout") > 0 {
            return Some(
                "Recent websocket DNS resolution timed out before upstream connect completed."
                    .to_string(),
            );
        }
        if runtime_doctor_marker_count(summary, "websocket_dns_overflow_reject") > 0 {
            return Some(
                "Recent websocket DNS resolution work was rejected after the overflow queue saturated."
                    .to_string(),
            );
        }
        if runtime_doctor_marker_count(summary, "websocket_dns_overflow_enqueue") > 0
    ...
    fn runtime_doctor_websocket_connect_diagnosis(
        summary: &RuntimeDoctorSummary,
    ) -> Option<String> {
        if runtime_doctor_marker_count(summary, "websocket_connect_local_pressure") > 0 {
            return Some(
                "Recent websocket connect failed due local pressure before upstream commit."
                    .to_string(),
            );
        }
        ...
        if runtime_doctor_marker_count(summary, "websocket_connect_overflow_enqueue") > 0 {
            return Some(format!(
                "Recent websocket connect overflow queueing was observed. Next step: {}",
                runtime_doctor_websocket_connect_overflow_next_step(summary)
            ));
        }
        if runtime_doctor_marker_count(summary, "websocket_connect_overflow_dispatch") > 0 {
            return Some(format!(
                "Recent websocket connect overflow dispatch was observed. Next step: {}",
                runtime_doctor_websocket_connect_overflow_next_step(summary)
            ));
        }
        if runtime_doctor_marker_count(summary, "websocket_proxy_tunnel_failure") > 0 {
            return Some("Recent websocket upstream proxy tunnel failed before the upstream websocket handshake completed. Next step: inspect HTTPS_PROXY/NO_PROXY and the proxy's CONNECT support.".to_string());
        }
        None
    }

    fn runtime_doctor_profile_pressure_diagnosis(summary: &RuntimeDoctorSummary) -> Option<String> {
        if runtime_doctor_marker_count(summary, "profile_inflight_saturated") > 0 {
            let profile =
        ...
        if runtime_doctor_marker_count(summary, "profile_bad_pairing") > 0 {
            return Some("Recent route-specific bad pairing memory is steering fresh selection away from a flaky account.".to_string());
        }
        ...
        if runtime_doctor_marker_count(summary, "local_rewrite_provider_auth_failure") > 0 {
    ...
    fn runtime_doctor_continuation_diagnosis(summary: &RuntimeDoctorSummary) -> Option<String> {
        if runtime_doctor_marker_count(summary, "compact_fresh_fallback_blocked") > 0 {
            return Some("Recent compact lineage guard failed closed so a follow-up stayed owner-first until upstream continuity was proven dead.".to_string());
        }
        if runtime_doctor_marker_count(summary, "compact_pressure_shed") > 0 {
            return Some("Recent pressure mode is shedding fresh compact requests to preserve continuation-heavy traffic.".to_string());
        }
        if runtime_doctor_marker_count(summary, "chain_dead_upstream_confirmed") > 0 {
            return Some(format!(
        ...
        if runtime_doctor_marker_count(summary, "stale_continuation") > 0 {
            return Some(format!(
        ...
        if runtime_doctor_marker_count(summary, "chain_retried_owner") > 0 {
            return Some(format!(
        ...
        if runtime_doctor_marker_count(summary, "previous_response_fresh_fallback") > 0 {
        ...
        if runtime_doctor_marker_count(summary, "previous_response_not_found") > 0 {
            return Some(format!(
        ...
        if runtime_doctor_marker_count(summary, "compact_final_failure") > 0 {
        ...
        let compact_exit_counts = runtime_doctor_compact_exit_counts(summary);
        if !compact_exit_counts.is_empty() {
            return Some(format!(
                "Recent compact exit paths were logged: {}.",
                runtime_doctor_count_breakdown(&compact_exit_counts)
            ));
        }
        None
    }

    fn runtime_doctor_compatibility_diagnosis(summary: &RuntimeDoctorSummary) -> Option<String> {
        if summary.compat_warning_count > 0 {
            return Some(format!(
        ...
        if summary.persisted_dead_continuations > 0 {
            return Some(format!(
        ...
        if runtime_doctor_marker_count(summary, "websocket_reuse_watchdog") > 0 {
            return Some("Recent websocket session reuse degraded before a terminal event; fresh reuse may be steering away from that profile.".to_string());
        }
        if runtime_doctor_marker_count(summary, "profile_auth_recovered") > 0 {
            return Some(format!(
                "Recent profile auth recovered after an upstream unauthorized response. Next step: {}",
                runtime_doctor_profile_auth_recovery_next_step(summary)
            ));
        }
        if runtime_doctor_marker_count(summary, "precommit_budget_exhausted") > 0 {
            return Some("Recent candidate selection exhausted before commit.".to_string());
        }
        if runtime_doctor_marker_count(summary, "upstream_usage_limit_passthrough") > 0
        ...
        {
            return Some("Recent quota hardening skipped near-exhausted sends or passed through upstream usage-limit responses.".to_string());
        }
        if runtime_doctor_marker_count(summary, "local_rewrite_gemini_quota_rotate") > 0
    ...
    fn runtime_doctor_provider_diagnosis(summary: &RuntimeDoctorSummary) -> Option<String> {
        if runtime_doctor_marker_count(summary, "local_rewrite_provider_model_fallback") > 0 {
            let provider = runtime_doctor_marker_last_field(
                summary,
        ...
        if runtime_doctor_marker_count(summary, "local_rewrite_gemini_invalid_stream_retry") > 0
        ...
        if runtime_doctor_marker_count(summary, "local_rewrite_gemini_compact_fallback") > 0 {
            let reason = runtime_doctor_marker_last_field(
                summary,
        ...
        if runtime_doctor_marker_count(summary, "local_rewrite_gemini_live_error") > 0
        ...
        {
            return Some("Recent Gemini Live bridge errors were observed; inspect local_rewrite_gemini_live_* markers for the failing profile/request.".to_string());
        }
        None
    }

    fn runtime_doctor_transport_diagnosis(summary: &RuntimeDoctorSummary) -> Option<String> {
        if runtime_doctor_marker_count(summary, "stream_read_error") > 0 {
            return Some("Recent upstream stream read failure detected after commit.".to_string());
        }
        if runtime_doctor_marker_count(summary, "local_writer_error") > 0 {
            return Some(
                "Recent local writer failure detected while forwarding an upstream stream."
                    .to_string(),
            );
        }
        if runtime_doctor_marker_count(summary, "upstream_connect_timeout") > 0
        ...
        {
            return Some("Recent upstream connect failures detected.".to_string());
        }
        if runtime_doctor_marker_count(summary, "state_save_error") > 0 {
            return Some("Recent runtime state save failures detected.".to_string());
        }
        if runtime_doctor_marker_count(summary, "state_save_queue_backpressure") > 0
    ...
    fn runtime_doctor_trailing_diagnosis(summary: &RuntimeDoctorSummary) -> Option<String> {
        ...
        if runtime_doctor_marker_count(summary, "profile_probe_refresh_error") > 0 {
            return Some("Recent background quota refresh failures detected; fresh selection may rely on stale quota snapshots.".to_string());
        }
        if runtime_doctor_marker_count(summary, "profile_probe_refresh_start") > 0 {
            return Some("Background quota refresh activity was detected; inspect the last marker for the most recent profile refresh.".to_string());
        }
        if runtime_doctor_marker_count(summary, "first_upstream_chunk") > 0
            && runtime_doctor_marker_count(summary, "first_local_chunk") == 0
        {
            return Some("Likely writer stall: upstream produced data but the local writer did not emit a first chunk in the sampled tail.".to_string());
        }
        if summary.runtime_broker_mismatch {
            return Some("A running runtime broker uses a different prodex binary than this command; restart active prodex/codex sessions so the patched runtime is loaded.".to_string());
        }
        if summary.prodex_binary_mismatch {
            return Some("Multiple prodex binaries on PATH differ by version or hash; align installs so new sessions use the patched runtime.".to_string());
        }
        if runtime_doctor_marker_count(summary, "selection_pick") > 0
            || runtime_doctor_marker_count(summary, "selection_skip_current") > 0
        {
            return Some("Recent selection decisions were logged; inspect the last marker for why a profile was picked or skipped.".to_string());
        }
        None
    }
}
