...
fn create_sqlite_schema(path: &std::path::Path) {
...
fn sqlite_migrations_are_explicit_and_forbidden_on_request_path() {
    ...
    assert_eq!(plan.migrations.len(), SQLITE_MIGRATIONS.len());
...
fn sqlite_external_migrator_open_is_the_only_ddl_eligible_mode() {
    ...
    assert_eq!(plan.required_schema_version, REQUIRED_SQLITE_SCHEMA_VERSION);
    assert!(plan.ddl_allowed);
    assert_eq!(plan.migration_count, SQLITE_MIGRATIONS.len());
}
...
#[test]
fn sqlite_atomic_reservation_allows_only_one_concurrent_claim_per_budget_scope() {
    let root = std::env::temp_dir().join(format!(
        "prodex-storage-sqlite-reserve-{}",
    ...
    conn.execute(
        ...
        rusqlite::params![tenant_id.to_string(), "tenant", 1_i64, 1_i64],
    )
    ...
    let run = |barrier: Arc<Barrier>| {
        ...
        std::thread::spawn(move || {
            ...
            barrier.wait();
            execute_sqlite_atomic_reservation(&path, &command)
        })
    };
    ...
    assert_eq!(
        results
            .iter()
            .filter(|result| { matches!(result, Err(rusqlite::Error::StatementChangedRows(0))) })
            .count(),
        1
    );
