...
fn prepare_managed_codex_home_merges_split_legacy_symlink_targets_before_relinking() {
    ...
    create_codex_home_if_missing(&second_profile_home).expect("second profile home should exist");
    ...
    create_codex_home_if_missing(&current_session_dir).expect("current session dir should exist");
    create_codex_home_if_missing(&legacy_session_dir).expect("legacy session dir should exist");
