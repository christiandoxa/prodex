...
def routing_score_weighted_total(
    ...
    return (
        ...
        + available_capacity * load_weight
        + cost * cost_weight
...
def routing_score_batch(
    ...
    if weight_total <= 0 or weight_total > ROUTING_SCORE_SCALE:
        return 2
    if (
        not routing_score_valid(health_weight)
        or not routing_score_valid(load_weight)
        or not routing_score_valid(cost_weight)
        or not routing_score_valid(latency_weight)
        or not routing_score_valid(risk_weight)
        or not routing_score_valid(priority_weight)
        or not routing_score_valid(affinity_weight)
    ):
        return 2
    ...
    for index in range(count):
        ...
        if (
            not routing_score_valid(health_value)
            or not routing_score_valid(load_value)
            or not routing_score_valid(cost_value)
            or not routing_score_valid(latency_value)
            or not routing_score_valid(risk_value)
            or not routing_score_valid(priority_value)
        ):
            return 3
        ...
        normalized_values[unsafe_offset=base + 2] = routing_score_inverse(
            cost_value
        )
        normalized_values[unsafe_offset=base + 3] = routing_score_inverse(
            latency_value
        )
        normalized_values[unsafe_offset=base + 4] = routing_score_inverse(
            risk_value
        )
...
def prodex_routing_plan_batch(
    ...
    var status = routing_score_batch(
        health,
        load,
        quota_headroom,
        quota_present,
    ...
    for index in range(count):
        ...
        if (
            (hard_value != 0 and hard_value != 1)
            or capability_value < 0
            or capability_value > 255
        ):
            return 4
        ...
        if hard_value == 0:
            ...
            capability_value & required_capability_mask
    ...
    for position in range(ordered_count):
        ...
        for candidate_position in range(position + 1, ordered_count):
            var candidate_index = ordered_indices[
                unsafe_offset=candidate_position
            ]
            var candidate_is_better = False
            if (
                affinity[unsafe_offset=candidate_index]
                > affinity[unsafe_offset=best_index]
            ):
                ...
                if (
                    scores[unsafe_offset=candidate_index]
                    > scores[unsafe_offset=best_index]
                ):
                    candidate_is_better = True
                elif (
                    scores[unsafe_offset=candidate_index]
                    ...
                    if (
                        provider_order[unsafe_offset=candidate_index]
                        < provider_order[unsafe_offset=best_index]
                    ):
                        candidate_is_better = True
                    elif (
                        provider_order[unsafe_offset=candidate_index]
...
def prodex_capability_match_batch(
    ...
    if (
        count < 0
        or count > 64
        or required_capability_mask < 0
        or required_capability_mask > 127
    ):
        return 5
    ...
    for index in range(count):
        ...
        var capability_value = capability_masks[unsafe_offset=index]
        if (
            (well_formed_value != 0 and well_formed_value != 1)
            or capability_value < 0
            or capability_value > 127
        ):
            return 5
