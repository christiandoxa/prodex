...
The current local version in this repo is `0.231.0`:

```bash
npm install -g @christiandoxa/prodex@0.231.0
```
...
DeepSeek support is a translated compatibility path: text, function/MCP/local shell/apply-patch style tools, `tool_choice`, reasoning effort, JSON object mode, stop sequences, sampling, token limits, logprobs, streaming usage, and cache hit/miss usage are mapped to DeepSeek OpenAI Chat where possible. JSON Schema is degraded to JSON object mode and marked in response metadata, web search is not advertised as native; `[deepseek] web_search_mode = "auto"` preserves the current best-effort OpenAI Chat `web_search_options` forwarding with fallback, `"off"` rejects web-search tools, and `"openai_chat"` explicitly selects that best-effort path. `PRODEX_DEEPSEEK_WEB_SEARCH_MODE` provides the same setting for gateway/profileless use. The `anthropic` and `function_proxy` modes currently fail clearly until those adapters/backends exist. Image/document/audio/video message content is rejected, deprecated penalty fields are not mapped, and `parallel_tool_calls=false` is rejected because DeepSeek has no equivalent enforcement control. Strict function tools are opt-in beta: set `[deepseek] strict_tools = true` in the selected Codex profile config, or `PRODEX_DEEPSEEK_STRICT_TOOLS=1` for gateway/profileless use. Prodex then routes DeepSeek Responses rewrites through `https://api.deepseek.com/beta` unless `[deepseek] beta_base_url = "..."` or `PRODEX_DEEPSEEK_BETA_BASE_URL` overrides it, sets each function tool `strict: true`, and rejects unsupported strict schema keywords instead of silently dropping them. DeepSeek beta prefix completion and FIM are not enabled yet; `prefix`, `prompt`, and `suffix` completion-style requests fail fast.
