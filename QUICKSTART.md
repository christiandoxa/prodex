...
- **Live Mode:** The Gemini Live websocket bridge supports anonymous/personal compatible callers, authenticated gateway virtual keys, and adapter tests with bounded frames, session duration, token reservation, per-frame accounting, and final reconciliation. Codex 0.140.0 removed upstream TUI voice controls.
...
Gateway virtual-key realtime WebSocket sessions use bounded 32 KiB frames, a five-minute session ceiling, an upfront token reservation, per-frame accounting, and terminal usage reconciliation. One governed provider/profile remains fixed for the session, and policy denial or token exhaustion closes it without rotation.
