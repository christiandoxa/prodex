...
| `codex-rs/exec/src/exec_events.rs` | `ThreadEvent` | `codex exec --json` emits JSONL `thread.started`, `turn.started`, `item.*`, `turn.completed`, `turn.failed`, and `error` events. | `prodex ping openai` requires the thread/turn lifecycle, an `agent_message` equal to `PONG`, and `turn.completed`. |
