...
| `prodex-quota::render::{windows,gemini,pool,reports}` | Quota summaries, rendering, reset calculations | Mostly pure | `chrono`, terminal formatting | Medium | A/B | `MOVE_NOW` for normalized pool aggregation; `KEEP_RUST` for conversion/formatting | Main and OpenAI pool rows aggregate through Mojo; chrono, provider acquisition, labels, and rendering stay Rust |
...
| `prodex-app::runtime_external_provider_config::external_catalog_models` | Merge launch, dynamic, and provider model IDs | Pure after Rust catalog parsing | model metadata and provider tables | Medium | A/B | `MOVE_NOW` for ordered ID dedup; `KEEP_RUST` for parsing, metadata, limits, and JSON projection | Reuses `prodex_mojo_rich_catalog_merge_v1` with an empty canonical catalog; feature-off Rust oracle matches Unicode whitespace, ASCII case folding, stable order, and IDs longer than 65,536 bytes |
