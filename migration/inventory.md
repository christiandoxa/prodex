...
| `prodex-provider-core::{catalog,constraints,models}` | Provider capabilities and model constraints | Mostly pure | `serde`, provider data | High | A/B | `MOVE_NOW` for normalized constraints; `KEEP_RUST` for parsing/catalog/provider adapter | `prodex_provider_constraints_evaluate` is called by the active public constraint evaluator after Rust normalization; 2,000 generated differential cases and full provider-core suite pass |
...
| `prodex-runtime-{doctor,log,metrics,capabilities,tuning}` | Diagnostics, logs, metrics, policy tuning | Mixed | filesystem, time, terminal | High | B/C | `MOVE_NOW` for normalized tuning defaults; `KEEP_RUST` for diagnostics/capabilities/formatting | `prodex_runtime_tuning_defaults` is called by runtime config, probe refresh, websocket worker, and async/log default paths; environment parsing and diagnostics stay Rust |
