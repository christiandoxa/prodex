...
| --- | --- | --- |
| PostgreSQL schema | External migrations cover tenant/accounting plus four-kind governance revisions/pointers/approvals, execution approvals, sessions/revocation epochs, audit-chain metadata, SIEM outbox and dead letters. | Governance break-glass activation and legal-hold workflow remain disabled capabilities. |
...
| Drill execution | The disposable PostgreSQL/TLS/RLS proof and authenticated AES-256-GCM custom dump/isolated restore drill passed. Final synthetic RPO was 2.004 seconds and restore RTO was 1.442 seconds. | These synthetic values are not approved production RPO/RTO objectives; managed failover, WAL/PITR and regional cutover remain external drills. |
