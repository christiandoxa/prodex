...
| --- | --- | --- |
| PostgreSQL schema | `prodex-storage-postgres` defines two external migrations and twelve tenant/accounting tables: tenants, virtual keys, service identities, users, role bindings, provider credentials, budget counters, budget policies, reservations, usage ledger, audit log, and idempotency records. | Policy lifecycle, classification, provider registry, access sessions, revocations, execution approvals, SIEM delivery, legal-hold, and other governance entities are absent. |
...
| Migration boundary | `prodex-storage-postgres` rejects DDL in gateway startup and request-path modes. The Kubernetes manifest supplies a distinct migration Job and service account. | There is no complete expand/backfill/contract series for governance data, tested downgrade or forward-recovery procedure, migration serialization contract, or production rehearsal evidence. |
| SQLite | `prodex-storage-sqlite` mirrors the current twelve-table storage subset and uses transactional local operations. | SQLite does not provide the multi-replica HA or PostgreSQL RLS boundary required by the enterprise target. It does not contain the missing governance schema. |
...
| Backup and restore | `scripts/ci/backup-restore-drill.mjs` creates a PostgreSQL custom dump, restores it, compares fingerprints, checks a recovery-point marker, validates accounting totals, and tests RLS for the current twelve tables. It writes redacted JSON evidence. | The drill does not cover the future governance schema, policy and registry consistency, complete audit-chain verification, legal holds, SIEM outbox state, encryption-key separation, WAL/PITR, regional recovery, or application cutover. |
...
| Audit durability | The typed domain and storage layers define an append-only digest-linked audit model, and PostgreSQL/SQLite expose append plans. | Live application paths also use a separate JSONL audit log, and some production call sites discard append failures. There is no transactional audit-plus-SIEM-outbox write or restore-time chain-anchor verification. |
| Drill execution | The drill self-test and focused storage tests can run without an external database. | During this audit, the real PostgreSQL proof lacked a configured PostgreSQL URL or local client tools, and the disposable backup drill did not reach its published endpoint. Therefore no successful real database restore is claimed by this audit. |
