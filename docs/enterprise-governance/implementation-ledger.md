...
| P4-06 | Endpoint-aware circuit breaker and background health | tested | Governed dispatch records transport/overload/success outcomes into the existing route-specific health and circuit state consumed by immutable routing snapshots; `gateway_upstream_transport_failure_uses_stable_response_without_endpoint_details` and the production boundary guard cover the wiring |
...
| ID | Requirement | Status | Evidence or next proof |
| --- | --- | --- | --- |
| P5-01 | CLI, IDE, API, and supported machine channels share one authenticated application boundary | tested | Production/application boundary guards cover forwarded HTTP, compact, SSE, and Gemini Live compatibility routes; virtual-key realtime sessions fail closed because bounded per-frame token accounting is unavailable; the synthetic root bearer compatibility principal is restricted to personal mode |
