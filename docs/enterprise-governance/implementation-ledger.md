...
| P3-08 | Narrow break-glass with expiry and review | tested | Content-free break-glass approvals have durable create/approve/use/revoke/review lifecycle, exact tenant/principal/operation binding, expiry/use limits, mandatory audit, and an enforced retention-purge activation path |
...
| P5-02 | OIDC PKCE/device or supported human flow, bearer validation, service identity, and mTLS | tested | Gateway browser Authorization Code with PKCE S256, secure cookie login/callback/logout, workload JWT/JWKS/scope verification, Rustls client-certificate validation, and JWT `cnf.x5t#S256` peer binding are wired and covered by focused runtime tests; deployment IdP/PKI rotation remains acceptance evidence |
