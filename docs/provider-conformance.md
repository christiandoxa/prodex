...
- Copilot now has a dedicated provider-core translator with tested Responses request/response/stream chat-compat translation while `responses/compact` remains passthrough; request helpers, response-id extraction, and characterization tests are split under `translators/copilot/`
