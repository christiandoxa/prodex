...
- Copilot now publishes its native Responses wire contract through a dedicated passthrough translator for `responses` and `responses/compact`, preserving continuation metadata and native tool surfaces; request helpers, response-id extraction, and characterization tests remain split under `translators/copilot/`
