...
Use `npm run ci:release-hygiene` for the full release gate. By default it checks `HEAD`; use `-- --range main..HEAD` or `-- --base origin/main --head HEAD` for branch ranges. The runner executes changelog-noise, metadata-only, version metadata, empty release commit, duplicate release version, tag/changelog, release-run tests, release guard fixtures, and release-cut backcompat fixtures as one serial gate. Changelog-only refresh commits such as `docs(changelog): refresh ...` are rejected unless they are part of a release-like commit; let `npm run release:run -- --version <semver>` render the final `CHANGELOG.md` section in the release commit instead. Real fixes to changelog tooling, for example `fix(changelog): ...` commits that change `scripts/npm/changelog.mjs`, are not changelog-only and remain valid.
...
Use `npm run ci:size-guard` after Rust module reshaping. Besides per-file line caps, it now fails when one production directory accumulates too many near-limit sibling modules; the goal is domain ownership, not threshold-only file splits. Use `npm run ci:size-guard-fixtures` after changing the guard itself.
