...
Large structural extraction is allowed only when the commit message declares it explicitly with `Mechanical-only: yes` or `[mechanical-only]`. The declaration is for move/split/reshape commits only, so use a `refactor`, `test`, `chore`, or `ci` subject that says what was mechanically moved. For staged/worktree checks before committing, pass `-- --message-file .git/COMMIT_EDITMSG` or `-- --message "refactor: split foo"` when validating a prepared message. If behavior changes are needed, put them in a separate smaller commit after the mechanical extraction.
