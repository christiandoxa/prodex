...
| --- | --- | --- |
| `service_mode` | `gateway` | Typed process role. `gateway` preserves the data-plane policy contract. `control-plane` is accepted only by the dedicated control-plane entrypoint and requires production projected secrets, a projected `role = "admin"` token, shared PostgreSQL or Redis state, and a `required_scope = "control_plane"` workload-identity block as the native mTLS transport identity. Control-plane policies reject provider, data-plane auth, virtual-key, routing, SSO, outbound observability, request-constraint, and guardrail capabilities. Bank control-plane policies additionally require at least two distinct projected admin principals per explicit tenant. |
...
| Policy key | Default | Meaning |
| --- | --- | --- |
| `secrets.backend` | `file` | Backend for Prodex-owned runtime broker capability secrets: exact `file` or `keyring`. `PRODEX_SECRET_BACKEND` overrides policy. |
| `secrets.keyring_service` | empty | Exact non-empty, whitespace-free OS-keyring service name. Required only with `secrets.backend = "keyring"`; `PRODEX_SECRET_KEYRING_SERVICE` overrides policy. |
...
The file/keyring selector does not change Codex-owned profile `auth.json`, MCP
credentials, or production projected `SecretRef` resolution. When keyring is
selected, Prodex lazily migrates an existing runtime broker capability file
only after the keyring write succeeds; keyring account names contain a bounded
hash instead of the source path.

| Policy key | Environment override | Default | Meaning |
| --- | --- | --- | --- |
| `gateway.listen_addr` | none | `127.0.0.1:4000` | Gateway bind address. Non-loopback binds require `--auth-token`, `PRODEX_GATEWAY_TOKEN`, `gateway.auth_token_ref`, or `gateway.virtual_keys`. |
...
| `gateway.harness` | CLI `--harness` | `native` | Model-facing request policy: `native`, `minimal`, or `evaluated`. CLI selection wins; the resolved mode is fixed for the gateway lifetime. |
...
| `gateway.require_auth` | none | `false` | Require gateway bearer auth even on loopback. Token value comes from non-empty, whitespace-free `--auth-token`, non-empty, whitespace-free `PRODEX_GATEWAY_TOKEN`, `gateway.auth_token_ref`, or configured virtual keys. |
...
| `gateway.admin_tokens[].token_ref` | none | empty | Projected control-plane bearer-token reference. Exactly one of `token_ref` or development-only `token_env` is required. Resolved admin credential values must be distinct so two configured names cannot collapse into one effective principal. |
...
| `gateway.sso.browser_flow` | none | `false` | Enable OIDC Authorization Code browser login at `/v1/prodex/gateway/auth/login`, callback, CSRF-protected POST-only logout, and signed POST-only back-channel logout routes. State/session records are bounded in-process for local mode and shared through coordination Redis in the accepted multi-replica topology. State and session cookies are Secure, HttpOnly, and SameSite; the session-bound CSRF cookie is Secure and `SameSite=Strict` but intentionally script-readable so the dashboard can echo it in `x-csrf-token`. Sessions created before CSRF binding must authenticate again. |
...
| `gateway.workload_identity.enabled` | none | `false` | Enable service JWT verification before data-plane authorization. With `service_mode = "control-plane"`, this block is instead a transport-only native mTLS listener identity and does not enable workload JWT authentication. |
| `gateway.workload_identity.issuer` / `audience` | none | required for gateway service identity | Exact permitted HTTPS issuer and whitespace-free audience for data-plane workload tokens. These fields are forbidden in the control-plane transport-only form. |
...
| `gateway.workload_identity.required_scope` | none | `data_plane` for gateway | Exact role-scoped value: `data_plane` for the gateway service-identity form or `control_plane` for the dedicated transport-only form. Other combinations fail validation. |
| `gateway.workload_identity.mtls_required` | none | `false` | Require a verified client certificate. Data-plane workload authentication additionally binds the verified leaf to JWT `cnf.x5t#S256`; the control plane uses the certificate as its authenticated transport hop before admin bearer authorization. |
| `gateway.workload_identity.mtls_ca_ref` / `tls_identity_ref` | none | required when mTLS is enabled | Projected client trust bundle and combined server certificate/private-key PEM identity. The Rustls listener verifies the client chain and exposes only the SHA-256 peer-certificate fingerprint where workload JWT authentication is active. Bank gateway mode requires the JWT+mTLS form; bank control-plane mode requires the transport-only form. |
...
| `gateway.virtual_keys[].budget_usd` | none | empty | Optional persisted spend cap for estimated request cost when catalog or policy cost is available. In enforcement modes, authoritative input and output prices are mandatory; requests without an explicit output limit reserve the selected model's full known token ceiling and unknown ceilings fail closed. |
| `gateway.virtual_keys[].request_budget` | none | empty | Optional persisted total request cap for the virtual key name. |
| `gateway.virtual_keys[].rpm_limit` / `gateway.virtual_keys[].tpm_limit` | none | empty | Optional per-minute request/token caps. Personal/observe modes retain the semantic estimator. Enforcement modes use the explicit output limit when present; otherwise they conservatively reserve the selected model's full known token ceiling and fail closed for unknown ceilings. |
...
- OpenAI `additional_rate_limits` are preserved as independent backend buckets, including their
  explicit `allowed`, `limit_reached`, `normalModelSlug`, and unknown future fields. The pinned
  Codex `rust-v0.154.0` contract exposes `ordinaryUsageAllowed` on the account rate-limit
  response and `normalModelSlug` on each rate-limit snapshot. Prodex preserves both as backend
  metadata, but still requires an explicit source-backed Luna Reserve identifier before routing
  on that bucket. When an upstream bucket explicitly identifies itself as `Luna Reserve`, it is
  applicable only to Luna requests; Sol and Terra never use it. Reserve is a fallback capacity
  mode, not a model identifier accepted from requests, and an unlabeled
