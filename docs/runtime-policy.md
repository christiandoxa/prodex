...
- OpenAI `additional_rate_limits` are preserved as independent backend buckets, including their
  ...
  future fields. The release-qualified Codex reference currently uses `rust-v0.157.1`; runtime compatibility is capability-based and accepts Codex 0.153.2 or newer when required app-server surfaces are present. That reference exposes backend admission and model
