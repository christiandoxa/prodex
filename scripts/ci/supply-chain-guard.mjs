...
export function validateReleaseContainerPublication(contents) {
  ...
  for (const marker of [
    ...
    "d8dd659c11c156002942f09443b6cb7904959004d4e7f1f73777fdbb2d909b54",
...
export function validateWorkflow(filePath, contents) {
  ...
  const exactToolchains = contents.match(/^\s*toolchain:\s*1\.98\.1\s*$/gmu)?.length ?? 0;
  if (rustActions !== exactToolchains) {
    violations.push(`${filePath}: every rust-toolchain action must install exact toolchain 1.98.1`);
  }
...
export function validateDockerfile(contents, rustToolchain = "1.98.1") {
...
function selfTest() {
  ...
  assert.deepEqual(
    validateDockerfile("FROM rust:1.98.1@sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef AS builder\n"),
    [],
  );
...
async function main() {
  ...
  for (const marker of ['channel = "1.98.1"', 'components = ["clippy", "rustfmt"]']) {
