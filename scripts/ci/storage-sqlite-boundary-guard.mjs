...
export function validateManifest(tomlText, manifestPath = MANIFEST) {
  ...
  for (const dep of dependencies) {
    if (!ALLOWED_DEPENDENCIES.has(dep)) {
      errors.push(`${manifestPath}: [dependencies] must stay SQLite-plan boundary crates only; unexpected dependency '${dep}'`);
    }
    if (FORBIDDEN_DEPENDENCIES.has(dep)) {
      errors.push(`${manifestPath}: prodex-storage-sqlite cannot depend on forbidden driver/runtime/framework crate '${dep}'`);
    }
  }
  for (const dep of devDependencies) {
    ...
    if (FORBIDDEN_DEPENDENCIES.has(dep)) {
      errors.push(`${manifestPath}: prodex-storage-sqlite tests cannot depend on forbidden driver/runtime/framework crate '${dep}'`);
    }
  }
  for (const sectionName of sorted(sections.keys())) {
    if (sectionName.startsWith("target.")) {
      errors.push(`${manifestPath}: target-specific dependencies are not allowed in prodex-storage-sqlite`);
    }
  }
  return errors;
}
  ...
  sourceText.split(/\r?\n/u).forEach((line, index) => {
    for (const { name, pattern } of FORBIDDEN_SOURCE_PATTERNS) {
      if (pattern.test(line)) {
        errors.push(`${sourcePath}:${index + 1}: prodex-storage-sqlite cannot use ${name} boundary '${line.trim()}'`);
      }
    }
  });
  if (sourcePath === LIB && !sourceText.includes("#![forbid(unsafe_code)]")) {
    errors.push(`${sourcePath}: prodex-storage-sqlite crate root must forbid unsafe code`);
  }
  return errors;
}
...
`;
  ...
  assertSelfTest(
    validateManifest(`${valid}\nrusqlite = "0.40"\n`, "invalid/Cargo.toml").some((error) => error.includes("rusqlite")),
    "rusqlite dependency accepted",
  );
  assertSelfTest(
  ...
  assertSelfTest(
    validateSource("let conn = rusqlite::Connection::open(path);", "bad.rs").some((error) => error.includes("database driver")),
...
main().catch((error) => {
  ...
  process.stderr.write(`storage-sqlite-boundary-guard: ${message}\n`);
  process.exitCode = 1;
});
