...
const ALLOWLIST = Object.freeze([
  ...
  },
  {
    ...
    file: "crates/prodex-app/src/runtime_launch/proxy_startup/local_rewrite_gateway_file_ledger.rs",
    id: "blocking-disk-io",
    pattern: /\bstd::fs::(?:read|create_dir_all|rename|symlink_metadata)\s*\(/,
    maxHits: 5,
    reason:
      "gateway file ledger I/O is limited to bounded ledger loading, symlink rejection, append setup, and atomic summary persistence",
  },
  {
  ...
  {
    name: "local-rewrite-observability-background-export",
    file: "crates/prodex-app/src/runtime_launch/proxy_startup/local_rewrite_transport.rs",
    id: "spawn-blocking",
    pattern: /\.spawn_blocking\(move \|\| \{/,
    maxHits: 1,
    reason: "JSONL and blocking HTTP observability exports run after admission on the bounded blocking pool",
  },
  {
