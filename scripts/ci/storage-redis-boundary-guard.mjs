...
const MANIFEST = "crates/prodex-storage-redis/Cargo.toml";
const SRC_DIR = "crates/prodex-storage-redis/src";
const LIB = "crates/prodex-storage-redis/src/lib.rs";
...
const FORBIDDEN_SOURCE_PATTERNS = Object.freeze([
  ...
  { name: "json whole-map storage", pattern: /\b(serde_json|cjson)\b/u },
  { name: "whole-list rewrite", pattern: /\b(LRANGE|LSET|DEL|RPUSH)\b/u },
...
export function validateManifest(tomlText, manifestPath = MANIFEST) {
  ...
  for (const dep of dependencies) {
    if (!ALLOWED_DEPENDENCIES.has(dep)) {
      errors.push(`${manifestPath}: [dependencies] must stay Redis-plan boundary crates only; unexpected dependency '${dep}'`);
    }
    if (FORBIDDEN_DEPENDENCIES.has(dep)) {
      errors.push(`${manifestPath}: prodex-storage-redis cannot depend on forbidden driver/runtime/framework/JSON crate '${dep}'`);
    }
  }
  for (const dep of devDependencies) {
    ...
    if (FORBIDDEN_DEPENDENCIES.has(dep)) {
      errors.push(`${manifestPath}: prodex-storage-redis tests cannot depend on forbidden driver/runtime/framework crate '${dep}'`);
    }
  }
  for (const sectionName of sorted(sections.keys())) {
    if (sectionName.startsWith("target.")) {
      errors.push(`${manifestPath}: target-specific dependencies are not allowed in prodex-storage-redis`);
    }
  }
  return errors;
}
  ...
  sourceText.split(/\r?\n/u).forEach((line, index) => {
    for (const { name, pattern } of FORBIDDEN_SOURCE_PATTERNS) {
      if (pattern.test(line)) {
        errors.push(`${sourcePath}:${index + 1}: prodex-storage-redis cannot use ${name} boundary '${line.trim()}'`);
      }
    }
  });
  if (sourcePath === LIB && !sourceText.includes("#![forbid(unsafe_code)]")) {
    errors.push(`${sourcePath}: prodex-storage-redis crate root must forbid unsafe code`);
  }
  return errors;
}
...
`;
  ...
  assertSelfTest(
    validateManifest(`${valid}\nredis = "1"\n`, "invalid/Cargo.toml").some((error) => error.includes("redis")),
    "redis driver dependency accepted",
  );
  assertSelfTest(
    validateManifest(`${valid}\nserde_json = "1"\n`, "invalid-json/Cargo.toml").some((error) => error.includes("serde_json")),
    "JSON dependency accepted",
  );
  assertSelfTest(
    validateSource("let payload = serde_json::to_string(&state);", "bad.rs").some((error) => error.includes("json")),
    "whole-map JSON source accepted",
  );
  assertSelfTest(
    validateSource("redis.call('LRANGE', KEYS[1], 0, -1)", "bad.rs").some((error) => error.includes("whole-list")),
    "whole-list rewrite source accepted",
  );
  assertSelfTest(
    validateSource("redis.call('LSET', KEYS[1], 0, ARGV[1])", "bad.rs").some((error) => error.includes("whole-list")),
    "whole-list indexed rewrite source accepted",
  );
  assertSelfTest(
    validateSource("redis.call('RPUSH', KEYS[1], ARGV[1])", "bad.rs").some((error) => error.includes("whole-list")),
    "whole-list append rewrite source accepted",
  );
  assertSelfTest(
    validateSource("redis.call('INCRBY', KEYS[1], ARGV[1])", "good.rs").length === 0,
    "atomic Lua source rejected",
  );
  assertSelfTest(
    validateSource("#![forbid(unsafe_code)]\npub const LUA: &str = \"return 1\";", LIB).length === 0,
    "unsafe-forbidden crate root rejected",
  );
  assertSelfTest(
    validateSource("pub const LUA: &str = \"return 1\";", LIB).some((error) => error.includes("forbid unsafe code")),
...
main().catch((error) => {
  ...
  process.stderr.write(`storage-redis-boundary-guard: ${message}\n`);
  process.exitCode = 1;
});
