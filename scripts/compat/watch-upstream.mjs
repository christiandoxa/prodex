...
function diffField(pathLabel, baselineValue, currentValue, classification) {
...
function renderReport(snapshot, diffs, diffSummary) {
  const lines = [];
  ...
  for (const baselineFile of baseline.codex?.compatibility?.critical_files ?? []) {
    ...
    if (currentFile.required_contains_missing.length > 0) {
      diffs.push(makeDiff({
        ...
        ...BLOCKING_SEMANTIC_DIFF,
        label: DIFF_LABELS.REQUIRED_CONTENT_MISSING,
        metadata: {
          upstream: "codex",
          file: baselineFile.path,
        },
  ...
  for (const baselineCheck of baseline.codex?.compatibility?.semantic_checks ?? []) {
    ...
    for (const field of SEMANTIC_DIFF_FIELDS) {
      ...
      diffs.push(makeDiff({
        ...
        ...BLOCKING_SEMANTIC_DIFF,
        label: DIFF_LABELS.SEMANTIC_REQUIRED_CONTENT_MISSING,
        metadata: {
          upstream: "codex",
          check_id: baselineCheck.id,
          file: baselineCheck.file,
...
async function main() {
  ...
  const report = {
    ...
    exitCode: diffSummary.exit_code,
  ...
  const rendered = renderReport(current, diffs, diffSummary);
  if (args.json) {
  ...
  if (diffSummary.exit_code > 0) {
    process.exitCode = diffSummary.exit_code;
  }
}
