...
test("release-run dry-run covers mandatory release order without mutation or network", async () => {
  ...
  try {
    ...
    assertIncludesInOrder(stdout, [
      ...
      "dry-run: gh api --method GET /repos/example/prodex/actions/workflows/standalone-release.yml/runs?branch=main&per_page=20",
      "dry-run: gh api --method POST /repos/example/prodex/actions/workflows/standalone-release.yml/dispatches -f ref=main",
      "trigger-publish: would dispatch standalone-release.yml for main (0.2.0)",
      "release-run: watch-publish (0.2.0)",
      "dry-run: watch standalone-release.yml for ",
